...
* [Describe] Http工具类
* **************************/
namespace Junior.Utility.U9Utils
{
    #region Http连接操作帮助类
    /// <summary>
    /// Http连接操作帮助类
    ...
    public class HttpUtil : IDisposable
    {
        ...
        public HttpUtil(HttpClientRequest request = null)
        {
            if (request != null)
                httpRequest = request;
        }
        ...
        public static bool ContainsIgnoreCase(string @this, params string[] strs)
        {
        ...
        public static bool EqualIgnoreCase(string @this, params string[] strs)
        {
            if (strs!=null&&strs.Length>0)
            {
                foreach (var item in strs)
        ...
        public static bool StartsWithIgnoreCase(string @this, params string[] strs)
        {
            if (string.IsNullOrEmpty(@this))
                return false;
        ...
        public static Dictionary<string, string> ToDictionary( string @this)
        {
            var dic = new Dictionary<string, string>();
            @this = Regex.Replace(Regex.Replace(@this, @"^(.*)\?", ""), @"#(.*)$", "");
            string[] arr = @this.Split('&');
            if (arr!=null&&arr.Length>0)
            {
                foreach (var i in arr)
                {
                    ...
                    {
                        var t = i.Split('=');
                        //引用 System.Web by YJ 2021-09-01 12:02
                        dic.Add(t[0], System.Web.HttpUtility.UrlDecode(t[1]));
                    }
                }
            }
            return dic;
        }
        public static string Substring( string @this, string separator, bool lastIndexOf = true)
        {
        ...
        private void SetCer(HttpClientRequest req)
        {
            //这一句一定要写在创建连接的前面。使用回调的方法进行证书验证。
            if (StartsWithIgnoreCase(req.Url,"https"))
                ServicePointManager.ServerCertificateValidationCallback = (request, certificate, chain, errors) => true;

            //初始化对像，并设置请求的URL地址
            if ((WebRequest.Create(req.Url) as HttpWebRequest) == null)
                throw new ArgumentNullException("创建HttpWebRequest失败");
        ...
        /// </summary>
        /// <param name="req">http请求参数</param>
        private void SetCerList(HttpClientRequest req)
        {
            if (req.ClentCertificates?.Count > 0)
            {
        ...
        /// </summary>
        /// <param name="req">http请求参数</param>
        private void SetProxy(HttpClientRequest req)
        {
            var isIeProxy = false;
            if (!string.IsNullOrEmpty(req.ProxyIp))
                isIeProxy = ContainsIgnoreCase(req.ProxyIp,"ieproxy");
        ...
        /// </summary>
        /// <param name="req">http请求参数</param>
        private void SetCookie(HttpClientRequest req)
        {
            //设置Cookie字符串
        ...
        /// </summary>
        /// <param name="req">http请求参数</param>
        private void SetPostData(HttpClientRequest req)
        {
            //验证在得到结果时是否有传入数据
            if (!EqualIgnoreCase(request.Method,"get"))
            {
                ...
                //字符串
                else if (req.PostDataType == PostDataType.String && !string.IsNullOrEmpty(req.PostString))
                ...
                else if (req.PostDataType == PostDataType.File)
                {
                    ...
                    {
                        Dictionary<string,string> formDic = ToDictionary(req.PostString);
                    ...
                    //文件数据
                    if (req.PostFiles != null && req.PostFiles.Count > 0)
                      
                    {
                        foreach (var item in req.PostFiles)
                        {
                            if (File.Exists(item.Value))
                            {
                                formData.Append($"--{boundary}")
                                        .Append("\r\n")
                                        .Append($"Content-Disposition: form-data; name=\"{item.Key}\"; filename=\"{Substring(item.Value,Path.DirectorySeparatorChar.ToString())}\"")
                                        .Append("\r\n")
                    ...
                    {
                        string name = req.PostFileStreamInfo.name;
                        string fileName = req.PostFileStreamInfo.fileName;
        ...
        /// </summary>
        ///<param name="req">http请求参数</param>
        private void SetRequest(HttpClientRequest req)
        {
            //设置证书
            SetCer(req);
        ...
        private void SetEncoding(HttpClientRequest req, HttpResult result, byte[] responseByte)
        {
            //是否返回byte类型数据
        ...
        private byte[] GetByte(HttpClientRequest req, HttpResult result)
        {
            byte[] responseByte = null;
            var responseStream = response.GetResponseStream();

            //解压缩
            if (ContainsIgnoreCase(response.ContentEncoding,"gzip"))
                responseStream = new GZipStream(responseStream, CompressionMode.Decompress);
            else if (ContainsIgnoreCase(response.ContentEncoding,"deflate"))
        ...
        /// <param name="req">http请求参数</param>
        /// <param name="result">http返回参数</param>
        private void GetData(HttpClientRequest req, HttpResult result)
        {
            if (response == null)
                return;
        ...
        public HttpResult GetResult(HttpClientRequest req)
        {
            //返回参数
    ...
    #region Http请求参数类
    public class PostFileStreamInfo
    {
    ...
    public class HttpClientRequest
    {
        ...
        /// </summary>
        public PostFileStreamInfo PostFileStreamInfo { get; set; }
